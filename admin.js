...
async function loadTimetable() {
    const token = checkAuth();
    try {
        const response = await fetch(`${config.API_URL}/api/admin/timetable`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function deleteSlot(slotId) {
    ...
    const token = checkAuth();
    try {
        const response = await fetch(`${config.API_URL}/api/admin/timetable/${slotId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
async function loadTeachers() {
    const token = checkAuth();
    try {
        const response = await fetch(`${config.API_URL}/api/admin/teachers`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function handleEditSlot(slotId) {
    ...
    try {
        ...
        const [slotResponse, teachers] = await Promise.all([
            fetch(`${config.API_URL}/api/admin/timetable/${slotId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            }),
