...
function initializeUser() {
    ...
    if (user) {
        ...
        document.getElementById('welcomeMessage').textContent = 
            `Welcome ${user.name} (${user.subject || 'No Subject'} Teacher)`;
        
        if (!user.subject) {
            showError('No subject assigned to your account. Please contact admin.');
        }
    }
}
...
async function loadMyClasses() {
    ...
    try {
        ...
        const response = await fetch('https://timetablebackend-a7n9.onrender.com/api/timetable/teacher', {
            headers: {
        ...
        if (!slots || slots.length === 0) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="4" class="no-data">No classes scheduled for you</td>
                </tr>
            `;
            return;
        }
